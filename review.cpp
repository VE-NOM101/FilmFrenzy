...
void critic_review ::search(void)
{
    ...
    switch (ch)
    {
        getchar();
    case 'w':
        critic_review ::new_review();
        break;
    case 'e':
        critic_review ::edit_review();
        break;
    case 'd':
        critic_review ::delete_review();
        break;
    case 'v':
        critic_review ::view();
        break;
...
void add_watchlist(T &obj)
{
    ...
    char name[25];
    char wlist[50];
    if (typeid(obj) == typeid(user_review))
        strcpy(name, obj.login.curr_user);
    else if (typeid(obj) == typeid(critic_review))
        strcpy(name, obj.login.curr_critic);
    strcpy(wlist, name);
